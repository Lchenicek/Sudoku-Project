...
def row_unused(sudoku, board, row, num):
    for col in range(sudoku.row_length):
        if board[row][col] == num:
            return False
    return True

def col_unused(sudoku, board, col, num):
...
def test_fill_diagonals():
    ...
    for i in range(0, sudoku.row_length, sudoku.box_length):
        assert check_box(sudoku.get_board(), i, i) == True
...
def test_valid_in_row():
    ...
    for i in range(9):
        for j in range(1, 10):
            temp = row_unused(sudoku, board, i, j)
...
def test_valid_in_col():
    ...
    for i in range(9):
        for j in range(1, 10):
            temp = col_unused(sudoku, board, i, j)
...
def test_is_valid():
    ...
    def check_safe(sudoku, row, col, num):
        return row_unused(board, row, num) and col_unused(board, col, num) and unused_inbox(sudoku,
                                                                                            row - row % sudoku.box_length,
                                                                                            col - col % sudoku.box_length,
                                                                                            num)
